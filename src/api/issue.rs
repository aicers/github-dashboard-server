...
const TODO_LIST_PROJECT_NAME: &str = "to-do list";
const TODO_LIST_STATUS_DONE: &str = "Done";
...
impl Issue {
    ...
    ///   * The issue has at least one sub-issue and all of them are resolved
    pub(super) fn is_resolved(&self) -> bool {
        ...
        if (!self.project_items.nodes.is_empty())
            && self.project_items.nodes.iter().any(|project_item| {
                project_item.project_title == TODO_LIST_PROJECT_NAME
                    && project_item
                        .todo_status
                        .as_ref()
